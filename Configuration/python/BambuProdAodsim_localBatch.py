...
options.register('fileListPath', '../../scripts/ttjets.cfg',
    VarParsing.multiplicity.singleton,
    VarParsing.varType.string,
    'choose filelist path'
)

options.register('nFile', 0,
    VarParsing.multiplicity.singleton,
    VarParsing.varType.int,
    'choose number of the file'
)
...
nFiles = 0
filePath = ""
with open(options.fileListPath,'r') as fileList:
    for line in fileList:
        filePath = line[:-1] # strip newline
        if nFiles == options.nFile:
            break
        filePath = "" # so it's impossible to run over the last file over and over if options.nFiles is set too high
        nFiles += 1
filePath = "root://cmsxrootd.fnal.gov/" + filePath # point to redirector
print "file path:",filePath

process.source = cms.Source(
  "PoolSource",
  fileNames = cms.untracked.vstring(filePath)
)
...
MitTreeFiller.TreeWriter.fileName = 'bambu-output-file-tmp%.3i'%(options.nFile)
